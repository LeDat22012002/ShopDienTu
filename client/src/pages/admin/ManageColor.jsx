...
import { apiGetAllColors, apiDeleteColor } from '../../apis';
import { UpdateColor } from '.';
import { toast } from 'react-toastify';
...
const ManageColor = () => {
    ...
    // Delete Product
    const handleDeleteColor = (clid) => {
        Swal.fire({
            title: 'Are you sure...?',
            text: 'Are you sure remove this color ? ',
            icon: 'warning',
            showCancelButton: true,
        }).then(async (rs) => {
            if (rs.isConfirmed) {
                const responseDelete = await apiDeleteColor(clid);
                ...
                <div className="flex justify-end py-4">
                    <form className="w-[45%]">
                        <InputForm
                            ...
                            errors={errors}
                            fullWith
                            placeholder="Search name or hexCode color..."
                        />
                    </form>
                </div>
                <table className="w-full overflow-hidden text-left border-collapse rounded-lg shadow-md">
                    <thead className="text-sm text-white uppercase bg-gray-700">
                        <tr>
                            ...
                            <th className="px-4 py-2 ">hexCode</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-300">
                        {colors?.map((el, index) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-3 w-[200px]">
                                    <span>{el?.hexCode}</span>
                                </td>

                                <td className="px-4 py-3 space-x-2 text-center ">
                                    <div className="flex items-center justify-center gap-2">
                                        <span
                                            onClick={() => setEditColor(el)}
